...
if __name__ == "__main__":
    ...
    ap.add_argument("--LOSS_TYPE",               type=str,   default="sc_loss") 
    ...
    fp = f"{av.DATASET_NAME}_{av.TASK}_fhash_{av.LOSS_TYPE}{av.sc_subset_size}{av.fmap_mode}muse{av.m_use}{str_new}.pkl"
    #av.sc_subset_size=8
    av.ES = 50
    av.INIT_GAUSS = False
    ...
    def inner_foo(av,nohash_op,dval,d):
        if av.LOSS_TYPE == "sc_loss":
            av.C1_LAMBDA=dval
        else:
    ...
    for margin,v1,v2,v3 in variations:
        ...
        for dval in c1_val:
            ...
            p =  multiprocessing.Process(target=inner_foo, args=(av,nohash_op,dval,queue))
            procs.append(p)
            p.start()
